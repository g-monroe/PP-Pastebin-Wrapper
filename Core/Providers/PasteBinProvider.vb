...
Public Class PasteBinProvider
    ...
    Public Async Function EditPaste(paste As Paste, pastebinProvider As PasteBinProvider) As Task(Of String)
        'This is a Make Shift Edit. Goes By the Name of the Paste, not the ID. This isn't the official way to do it using the API, This code could be aboved upon, and isn't the best.
        Dim response = Await pastebinProvider.GetPastes()
        If (response.Success) Then
            Dim lines As String() = response.ContentData.Split(New String() {"</paste>"}, StringSplitOptions.None)
            For Each line As String In lines
                If line.Contains("<paste_title>") Then
                    Dim id As String = line.Split(New String() {"<paste_key>"}, StringSplitOptions.None).Last.Split(New String() {"</paste_key>"}, StringSplitOptions.None).First
                    Dim Name As String = line.Split(New String() {"<paste_title>"}, StringSplitOptions.None).Last.Split(New String() {"</paste_title>"}, StringSplitOptions.None).First
                    If Name = Paste.Name Then
                        Dim deleteResponse = Await pastebinProvider.DeletePaste(id)
                        If (deleteResponse.Success) Then
                            Dim createResponse = Await pastebinProvider.CreatePaste(Paste)
                            If (createResponse.Success) Then
                                Return "Success"
                            Else
...
Public Class ApiResponse
    ...
    Public Sub New(pasteBinProvider As PasteBinProvider, content As String, source As String)
        ContentData = content
        ...
        ElseIf (source = "Login") Then
            pasteBinProvider.ApiUserKey = content
        End If
    End Sub
End Class
